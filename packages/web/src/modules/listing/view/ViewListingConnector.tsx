...
> {
  ...
  closeModal(e: any) {
    ...
    if (e.target.id === "confirm") {
      this.props.history.push(
        `/listing/${this.props.match.params.listingId}/grab`
      );
    }
  }
  ...
  CopyToClipboard = (artist: string, venue: string) => {
    ...
    }ssr/?artist=${artist}&venue=${venue}&id=${
    ...
    if (this.copyText.current) {
      ...
      setTimeout(() => {
        this.copyText.current
          ? (this.copyText.current.className = "")
          : console.log("fuck");
      }, 5000);
    }
  };
          ...
          <div>
            {this.state.input === "ticky" && (
              <div>
                ...
                >
                  Are you sure you want to grab this ticky?
                </div>
                <div id="confirm-cancel-container">
                  <button id="cancel" onClick={this.closeModal}>
                    oH NOPE!
                  </button>
                  <button id="confirm" onClick={this.closeModal}>
                    YEP!
                  </button>
                </div>
              </div>
            ...
            {this.state.input === "copy" && (
              <div style={{ display: "block" }}>
                ...
                >
                  this link has been copied to your clippyboard :)
                </div>
                <div id="confirm-cancel-container">
                  <input
                    ...
                    defaultValue={`${
                      process.env.REACT_APP_SERVER_URL
                    }/?artist=${this.state.artist}&venue=${
                      this.state.venue
                    }&id=${listingId}`.replace(/ /g, "%20")}
                  />
                </div>
          ...
          {data => {
            ...
            return (
              <div id="viewlisting-container">
                <Helmet>
                  <title>
                    {data.listing.artist}-{data.listing.venue}
                  </title>
                  <meta name="description" content={`${data.listing.artist}`} />
                  <meta name="theme-color" content="#008f68" />
                </Helmet>
                <div id="viewlisting-artist">
                  {data.listing.artist.toUpperCase()}
                </div>
                <div id="viewlisting-venue">
                  {data.listing.venue.toUpperCase()}
                </div>
                ...
                >
                  ...
                  <span ref={this.copyText} id="copyText">
                    COPIED
                  </span>
                </div>
                <div id="viewlisting-footer">
                  {avail.length > 0 ? (
                    ...
                    <div
                      ...
                      id="ticky"
                      onClick={() => {
                        this.openModal("ticky", "null", "null");
                      }}
                    >
